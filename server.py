...
def index():
                ...
                intent = str(postRece['nlp']['reportehana'][0]['slug']) #Se obtiene el nombre del intent desde el post de Recast
                skill = str(postRece['conversation']['hana']) #Se obtiene el nombre del skill desde el post de Recast
                print(skill)
                print(intent)
                if intent == "reportehana" and skill == "hana": #Si el intent y el skill es el esperado
                               ...
                               data = jsonify(
                               status=200,
                               replies=[{'type': 'text','content': 'Perdón, no entendí. ¿Podrías refrasear tu pregunta?'}],conversation={'memory': { 'key': 'value' }} ) #Esta es la estructura de un mensaje simple en Recast, referir a: https://recast.ai/docs/concepts/structured-messages para conocer todos los formatos de mensaje permitidos
